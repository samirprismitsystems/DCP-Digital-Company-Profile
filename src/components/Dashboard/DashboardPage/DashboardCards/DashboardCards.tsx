...
import styles from "./styles/dashboardCards.module.scss";
export default function DashboardCards({ item }: any) {
  ...
  return (
    <div
      className={`text-center h-[190px] w-[190px] pt-8 pl-6 pr-6 pb-6 my-0 mx-auto rounded-3xl relative border-[3px] ${item.borderClass} bg-transparent after:bg-transparent after:w-[100px] after:h-[100px] after:border-solid after:border-[15px] ${item.afterBorderClass} after:border-t-0 after:border-r-0 after:rounded-t-none after:rounded-br-none after:rounded-bl-[20px] after:shadow-none after:top-auto after:-left-[10px] after:-bottom-[10px] hover:cursor-pointer after:right-auto after:z-[9] before:z-[-1] before:absolute after:absolute`}
      onMouseEnter={toggle}
      onMouseLeave={toggle}
    >
      <div className={`bg-info-light rounded-3xl absolute top-4 left-4 right-4 bottom-4 ${styles.cardShadow} flex flex-col items-center justify-center m-2`}>
        <div
        ...
        <span className={`${styles.dashboardCardFont} font-semibold w-full text-black text-[30px] mb-1 block`}>
          {item.digit}
        </span>
